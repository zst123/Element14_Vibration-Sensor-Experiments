...
#include "TFTLCD_Registers.h"

// Constructor for breakout board (configurable LCD control lines).
// Can still use this w/shield, but parameters are ignored.
Adafruit_TFTLCD::Adafruit_TFTLCD(PinName cs, PinName cd, PinName wr, PinName rd, PinName rst,
                  PinName d0, PinName d1, PinName d2, PinName d3,
                  PinName d4, PinName d5, PinName d6, PinName d7)
    : Adafruit_GFX(TFTWIDTH, TFTHEIGHT) {

  csPin = new DigitalInOut(cs);
  cdPin = new DigitalInOut(cd);
  wrPin = new DigitalInOut(wr);
  rdPin = new DigitalInOut(rd);
  rstPin = new DigitalInOut(rst);
  
  d0Pin = new DigitalInOut(d0);
  d1Pin = new DigitalInOut(d1);
  d2Pin = new DigitalInOut(d2);
  d3Pin = new DigitalInOut(d3);
  d4Pin = new DigitalInOut(d4);
  d5Pin = new DigitalInOut(d5);
  d6Pin = new DigitalInOut(d6);
  d7Pin = new DigitalInOut(d7);
...
#define delay(x) thread_sleep_for(x) // mbed os 6

// Control signals are ACTIVE LOW (idle is HIGH)
// Command/Data: LOW = command, HIGH = data
// These are single-instruction operations and always inline
#define RD_ACTIVE     rdPin->write(0)
#define RD_IDLE       rdPin->write(1)
#define WR_ACTIVE     wrPin->write(0)
#define WR_IDLE       wrPin->write(1)
#define CD_COMMAND    cdPin->write(0)
#define CD_DATA       cdPin->write(1)
#define CS_ACTIVE     csPin->write(0)
#define CS_IDLE       csPin->write(1)
#define RST_ACTIVE    rstPin->write(0)
#define RST_IDLE      rstPin->write(1)
...
void Adafruit_TFTLCD::begin(uint16_t id) {
  ...
  if ((id == 0x9325) || (id == 0x9328)) {
    ...
    while (i < sizeof(ILI932x_regValues) / sizeof(uint16_t)) {
      a = (uint16_t) (ILI932x_regValues[i++]);
      d = (uint16_t) (ILI932x_regValues[i++]);
      if (a == TFTLCD_DELAY)
        delay(d);
      else
        writeRegister16(a, d);
...
uint16_t Adafruit_TFTLCD::readPixel(int16_t x, int16_t y) {
  ...
  if (driver == ID_932X) {
    ...
    setWriteDir(); // Restore LCD data port(s) to WRITE configuration
    CS_IDLE;
    return (((uint16_t)r & 0b11111000) << 8) | (((uint16_t)g & 0b11111100) << 3) |
           (b >> 3);
  } else
    return 0;
}
...
void Adafruit_TFTLCD::write8(uint8_t value) {
  d0Pin->write((value & 0x01) != 0);
  d1Pin->write((value & 0x02) != 0);
  d2Pin->write((value & 0x04) != 0);
  d3Pin->write((value & 0x08) != 0);
  d4Pin->write((value & 0x10) != 0);
  d5Pin->write((value & 0x20) != 0);
  d6Pin->write((value & 0x40) != 0);
  d7Pin->write((value & 0x80) != 0);
...
uint8_t Adafruit_TFTLCD::read8fn(void) {
  ...
  uint8_t result;
  if (d0Pin->read()) { result |= 0x01; }
  if (d1Pin->read()) { result |= 0x02; }
  if (d2Pin->read()) { result |= 0x04; }
  if (d3Pin->read()) { result |= 0x08; }

  if (d4Pin->read()) { result |= 0x10; }
  if (d5Pin->read()) { result |= 0x20; }
  if (d6Pin->read()) { result |= 0x40; }
  if (d7Pin->read()) { result |= 0x80; }
